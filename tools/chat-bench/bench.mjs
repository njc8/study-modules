/* Benchmark a chat model on MATH 212 questions through OpenRouter.
   ...
   Reads OPENROUTER_API_KEY, CHAT_MODEL, and CHAT_REASONING from the repo's .env. Sends each question in
...
async function ask(messages){
  ...
  const r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',
    ...
    body:JSON.stringify({model:MODEL,messages:[{role:'system',content:SYSTEM_PROMPT},...messages],max_tokens:8000,...(env.CHAT_REASONING?{reasoning:{effort:env.CHAT_REASONING}}:{})})});
...
(async()=>{
  let md=`# Bench: ${MODEL}${env.CHAT_REASONING?' (reasoning '+env.CHAT_REASONING+')':''}\n\n`;
